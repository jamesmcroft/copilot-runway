...
// Resolve the absolute path of the worktrees root, expanding a leading
// `~` to the current user's home directory. Issue #44 requires every
// caller to read this from settings (no per project override). Returning
// an absolute path keeps downstream callers from re-implementing the
// expansion and keeps the test surface small.
function getWorktreesRoot() {
  let raw = resolveSetting('worktrees.root');
  if (typeof raw !== 'string' || raw.trim() === '') {
    raw = path.join(RUNWAY_DIR, 'worktrees');
  }
  raw = raw.trim();
