...
function canonicalizePath(p) {
  ...
  // Prefer realpathSync.native so Windows expands 8.3 short names like
  // C:\Users\RUNNER~1 into their long form. The pure-JS realpathSync
  // resolves symlinks but leaves short names alone.
  ...
  try {
    ...
    while (true) {
      ...
      cursor = parent;
      try {
        return path.join(real(cursor), ...tail);
      } catch {
        // keep walking
      }
    }
