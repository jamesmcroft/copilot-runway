...
function createLifecycleWatcher(emitter = new EventEmitter()) {
  ...
  function livenessSweep() {
    ...
    // Fallback discovery: only when the recursive fs.watch path is unavailable.
    // On supported platforms the watcher fires reconcileSession directly, so
    // skipping this avoids double-firing.
    if (recursiveWatchOk) return;
    if (!fs.existsSync(SESSION_STATE_DIR)) return;
    let entries;
    try {
      entries = fs.readdirSync(SESSION_STATE_DIR, { withFileTypes: true });
    ...
    for (const entry of entries) {
      ...
      reconcileSession(entry.name);
    }
    for (const sessionId of known.keys()) {
      if (!seenIds.has(sessionId)) reconcileSession(sessionId);
    }
  }

  let watcher = null;
  let sweepTimer = null;
  let recursiveWatchOk = true;
  ...
  function start() {
    ...
    try {
      ...
      // Recursive fs.watch is unsupported on some platforms (notably Linux
      // before Node 20). Fall back to a periodic directory scan inside the
      // existing liveness sweep instead.
      recursiveWatchOk = false;
      console.warn('[copilot-runway] session-state watcher unavailable, falling back to periodic scan; recursive fs.watch requires Node 20+ on Linux. Reason: ' + err.message);
    }
  ...
  // Build a point-in-time snapshot of every session currently visible on disk,
  // pre-classified into the same shape live emissions use. Each entry's `data`
  // matches the payload that would have been emitted by the corresponding
  // `session.active` / `session.inactive` event. Liveness is determined by the
  // same path the watcher itself relies on (getSessionStatus -> process.kill(pid, 0)).
  function snapshot() {
    const out = [];
    if (!fs.existsSync(SESSION_STATE_DIR)) return out;
    let entries;
    try {
      entries = fs.readdirSync(SESSION_STATE_DIR, { withFileTypes: true });
    } catch {
    ...
    for (const entry of entries) {
      ...
      const { status, pid } = getSessionStatus(sessionId);
      if (status === 'active') {
        out.push({ type: 'session.active', data: { sessionId, pid, at } });
      } else {
        out.push({ type: 'session.inactive', data: { sessionId, at } });
      }
    }
    return out;
  }

  return { emitter, start, stop, snapshot };
}
