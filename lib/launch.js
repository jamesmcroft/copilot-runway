...
//   and macOS osascript). Only server-trusted values (the session's cwd
//   and id, both sourced from the session-store DB) get interpolated into
//   shell strings, and they are quoted.
...
function buildTerminalCommands(platform, cwd, sessionId, env) {
  if (platform === 'win32') {
    const cmdString = `cd /d ${quoteCmd(cwd)} && copilot --resume=${sessionId}`;
    return [
      {
        bin: 'wt',
        args: ['-d', cwd, 'copilot', '--resume', sessionId],
  ...
  if (platform === 'darwin') {
    ...
    const script = `tell application "Terminal" to do script "cd \\"${escCwd}\\" && copilot --resume=${sessionId}"`;
    return [
      {
        bin: 'osascript',
        args: ['-e', script],
  ...
  const shellCmd = `cd "${escPosixShell(cwd)}" && copilot --resume=${sessionId}; exec $SHELL`;
