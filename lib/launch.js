...
const { RUNWAY_DIR, LAUNCHERS_FILE } = require('./paths');
const { isPathWithinProject } = require('../public/path-match');
const runwaySettings = require('./runway/settings');
...
// Default reader for the legacy launchers.json. The settings module
// owns the deprecation log (logged once per process the first time
// settings boots and folds the file into settings.json). This reader
// remains exported because the launch tests inject their own copy to
// stub the on-disk state.
function defaultReadLaunchers() {
  try {
    const raw = fs.readFileSync(LAUNCHERS_FILE, 'utf8');
    const parsed = JSON.parse(raw);
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) return parsed;
    console.warn(`[runway] ${LAUNCHERS_FILE} is not a JSON object, ignoring`);
    return {};
  } catch (err) {
    if (err && err.code === 'ENOENT') return {};
    console.warn(`[runway] failed to read ${LAUNCHERS_FILE}: ${err.message}`);
    return {};
  }
}

// Resolve the VS Code binary for a launch request.
//
// Order:
//   1. Caller-supplied launchers reader. Tests inject their own; in
//      production this surfaces the legacy launchers.json content so the
//      resolver can honour it if settings.json has not yet been written.
//   2. New settings model (`launchers.vscode`), with per-project override
//      when `projectKey` is provided.
//   3. Fallback to `code`.
//
// We prefer the explicit caller-injected reader when it returns a
// non-empty allowlisted value because (a) it preserves the existing
// test contract and (b) `~/.runway/settings.json` is auto-populated
// from launchers.json on first boot, so the two paths converge after
// the first run.
function resolveVscodeBinary(readLaunchers, projectKey) {
  let cfg;
  try {
    cfg = readLaunchers() || {};
  } catch (err) {
    console.warn(`[runway] launchers reader threw: ${err.message}, falling back to settings`);
    cfg = {};
  }
  const requested = typeof cfg.vscode === 'string' ? cfg.vscode.trim() : '';
  if (requested) {
    if (VSCODE_BINARIES.includes(requested)) return requested;
    console.warn(`[runway] vscode binary "${requested}" not in allowlist, falling back to settings`);
  }

  try {
    const resolved = runwaySettings.resolveSetting('launchers.vscode', projectKey);
    if (typeof resolved === 'string' && VSCODE_BINARIES.includes(resolved)) return resolved;
  } catch (err) {
    console.warn(`[runway] settings resolver failed for launchers.vscode: ${err.message}`);
  }
  return 'code';
}
...
function buildVscodeCommand(bin, cwd, platform, targetPath) {
  ...
  // each path ourselves to survive paths with spaces.
  //
  // When `targetPath` is provided the caller has already validated it
  // resolves inside `cwd` (see resolveTargetPath). We still pass `cwd`
  // first so VS Code opens (or reuses) a window rooted at the project
  // and then opens the file inside that window.
  const args = platform === 'win32'
    ? [quoteWinShellArg(cwd)]
    : [cwd];
  if (targetPath) {
    args.push(platform === 'win32' ? quoteWinShellArg(targetPath) : targetPath);
  }
  return {
    bin,
    args,
...
// Validate that an optional client-supplied file path lies inside the
// session's working directory. Returns:
//   { ok: true, absolute }      - path resolved and stays under cwd.
//   { ok: true, absolute: null } - no path was requested.
//   { ok: false, error, hint }  - rejected; caller should 400.
//
// Resolution rules:
//   - Reject anything that is not a non-empty string.
//   - Relative paths are resolved against `cwd`.
//   - Absolute paths are accepted as-is and then checked against `cwd`.
//   - After resolving, the absolute path must be cwd itself or a
//     descendant (isPathWithinProject already guards `foo` vs `foo-bar`
//     boundary bugs).
//   - No symlink traversal check: per-user 127.0.0.1 server, the path
//     comes from the same session-store DB row the server trusts; a
//     malicious cwd would already imply local code execution.
function resolveTargetPath(rawPath, cwd) {
  if (rawPath === undefined || rawPath === null || rawPath === '') {
    return { ok: true, absolute: null };
  }
  if (typeof rawPath !== 'string') {
    return { ok: false, error: 'invalid-path', hint: 'path must be a string' };
  }
  ...
  let absolute;
  try {
    absolute = path.isAbsolute(trimmed) ? path.resolve(trimmed) : path.resolve(cwd, trimmed);
  } catch (err) {
    return { ok: false, error: 'invalid-path', hint: err.message };
  }
  if (!isPathWithinProject(absolute, cwd)) {
    return {
      ok: false,
      error: 'path-outside-cwd',
      hint: `Refusing to open a path outside the session working directory: ${absolute}`,
    };
  }
...
//
// 1. PowerShell's `param(...)` only binds when the script is invoked via
//    `-File path.ps1 <args>` or when the body is wrapped in a script
//    block: `& { param(...) ... } <args>` inside a single -Command
//    string. Tokens that appear AFTER the -Command string are consumed
//    by PowerShell.exe itself, not handed to the script. We use the
//    script-block form so `$TargetPid` actually binds. The pid value is
//    substituted at the `__PID__` marker by the caller; the upstream
//    `pid` check guarantees a numeric value, so substitution is safe.
//
// 2. `SetForegroundWindow` refuses calls from background processes
//    (Node server is not in the foreground). The widely-used workaround
//    is to temporarily attach our input thread to the current foreground
//    window's thread (AttachThreadInput), which transfers "foreground
//    privilege" for the duration. We also call `BringWindowToTop` and
//    `ShowWindowAsync(SW_RESTORE)` first to maximise success on
//    minimised or obscured windows. Exit codes: 0 success, 1 no window
//    handle after the parent walk, 2 SetForegroundWindow refused.
//
// The parent walk (up to 5 hops) is needed because copilot.exe is a
// console child of its hosting terminal so its own MainWindowHandle is
// usually zero; the terminal a few hops up owns the visible window.
const WIN_FOCUS_PS = `
& {
  param([int]$TargetPid)
  Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;public class W{[DllImport("user32.dll")]public static extern bool SetForegroundWindow(IntPtr h);[DllImport("user32.dll")]public static extern bool ShowWindowAsync(IntPtr h,int n);[DllImport("user32.dll")]public static extern bool BringWindowToTop(IntPtr h);[DllImport("user32.dll")]public static extern IntPtr GetForegroundWindow();[DllImport("user32.dll")]public static extern uint GetWindowThreadProcessId(IntPtr h, out uint pid);[DllImport("kernel32.dll")]public static extern uint GetCurrentThreadId();[DllImport("user32.dll")]public static extern bool AttachThreadInput(uint a, uint b, bool attach);}' -ErrorAction SilentlyContinue
  ...
  $fgPid = 0
  $fgThread = [W]::GetWindowThreadProcessId($fg, [ref]$fgPid)
  $curThread = [W]::GetCurrentThreadId()
  $attached = $false
  if ($fgThread -ne 0 -and $fgThread -ne $curThread) {
    $attached = [W]::AttachThreadInput($curThread, $fgThread, $true)
  }
  [W]::ShowWindowAsync($h, 9) | Out-Null
  [W]::BringWindowToTop($h) | Out-Null
  $ok = [W]::SetForegroundWindow($h)
  if ($attached) { [W]::AttachThreadInput($curThread, $fgThread, $false) | Out-Null }
  if ($ok) { Write-Output "OK"; exit 0 } else { exit 2 }
} -TargetPid __PID__
`.trim();
...
async function focusExisting(pid, platform, runFocusCommand) {
  ...
  if (platform === 'win32') {
    // pid is already type-checked as a number above, so __PID__ is only
    // ever replaced by a digit string. No injection vector.
    const script = WIN_FOCUS_PS.replace('__PID__', String(pid));
    const r = await runFocusCommand('powershell', [
      '-NoProfile', '-NonInteractive', '-Command', script,
    ]);
    if (r.code === 0 && /OK/.test(r.stdout || '')) return { focused: true };
    // exit 2 = SetForegroundWindow refused even after AttachThreadInput,
    // exit 1 / anything else = no window handle found.
    if (r.code === 2) return { focused: false, reason: 'foreground-blocked' };
...
function createLaunchRouter(options = {}) {
  ...
  router.post('/:id/launch/vscode', async (req, res) => {
    ...
    const rawPath = req.body && typeof req.body === 'object' ? req.body.path : null;
    const resolved = resolveTargetPath(rawPath, session.cwd);
    if (!resolved.ok) {
      return res.status(400).json({ ok: false, error: resolved.error, hint: resolved.hint });
    }

    // Pass the session cwd as the projectKey so a per-project override
    // for launchers.vscode keyed by that absolute path wins. Orphan keys
    // (no matching project entry) silently fall through to the global
    // value, which is the documented resolver contract.
    const bin = resolveVscodeBinary(readLaunchers, session.cwd);
    const command = buildVscodeCommand(bin, session.cwd, platform, resolved.absolute);

    const result = await attemptSpawn(spawn, [command]);
    if (result.ok) return res.json({ ok: true, bin, path: resolved.absolute });
    ...
    if (pid && isPidAlive(pid)) {
      ...
      if (result.ok) {
        ...
        if (platform === 'darwin') {
          ...
          if (reason === 'foreground-blocked') {
            hint = 'Windows blocked focus from the background; opened a new terminal instead.';
          } else {
            hint = 'Could not locate the existing terminal window; opened a new one.';
          }
        }
        return res.json({
          ok: true,
