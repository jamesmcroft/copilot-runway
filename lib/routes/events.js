...
function createEventsRouter(emitter, { heartbeatMs = 25000, snapshot } = {}) {
  ...
  router.get('/', (req, res) => {
    ...
    //
    // Note: snapshot() is synchronous today, so this buffer is defensive
    // and typically stays empty. If snapshot() ever becomes async, the
    // buffer becomes load-bearing: keep the subscribe-first / drain /
    // atomic-swap ordering intact.
    const buffer = [];
    ...
    // Track frames actually written so state.snapshot.end reports the
    // count subscribers can verify against, not the intended size.
    let sentCount = 0;
    ...
    // Defense in depth: Node normally emits `close` after `error`, but
    // wiring both guards against the rare case where a socket error never
    // produces a subsequent close. cleanup is idempotent: detachBuffer
    // short-circuits, clients.delete and clearInterval are safe to call
    // twice.
    const cleanup = () => {
