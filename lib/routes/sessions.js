...
const { getSessionAgent } = require('../runway/session-agents');
...
// Cap refs server-side. The card renders up to 3 linked badges plus an
// optional "+N more" pill, so 4 entries is enough to drive that math
// without shipping unbounded ref lists to every list response.
const REFS_MAX = 4;
...
FROM sessions s`;
    ...
    // card renders); commit refs would clutter the sidebar.
    ...
    const enriched = sessions.map(s => {
      ...
      const refs = refsStmt.all(s.id, REFS_MAX);
      return {
        ...rest,
        ...status,
